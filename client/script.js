...
function shbColor(event) {
    sayHelloButton.style.backgroundColor = 'black'
    //sayHelloButton.classList.add('cssClass')
    sayHelloButton.style.color = 'white'
    
}

sayHelloButton.addEventListener('mouseover', shbColor)
...
// PROBLEM 4
/*
    Now lets see if we can make a request to our server whedn we click the button
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/"blob"')
        .then((res) => {
            console.log(res.data)
...
function request() {
    axios.get('http://localhost:3000/query-test')
        .then((res) => {
            console.log(res.data)
            document.querySelector('#repeat-text')
        })
